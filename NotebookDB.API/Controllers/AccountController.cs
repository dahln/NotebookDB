...
namespace NotebookDB.API.Controllers
{
    public class AccountController : Controller
    {
        ...
        async public Task<IActionResult> DeleteAccount()
        {
            ...
            List<string> typesDeleted = new List<string>();
            var notebooks = await _db.Notebooks.Include(x => x.AuthorizedUsers).Where(x => x.AuthorizedUsers.Any(y => y.UserId == userId && y.IsNotebookAdmin)).ToListAsync();
            foreach(var notebook in notebooks)
            {
                if(notebook.AuthorizedUsers.Count(x => x.IsNotebookAdmin) == 1)
                {
                    typesDeleted.Add(notebook.Id);
                    var values = _db.Values.Where(x => x.Instance.NotebookId == notebook.Id);
                    var instances = _db.Instances.Where(x => x.NotebookId == notebook.Id);
                    var files = _db.Files.Where(x => x.Instance.NotebookId == notebook.Id);
                    var fields = _db.Fields.Where(x => x.NotebookId == notebook.Id);
                    var sections = _db.Sections.Where(x => x.NotebookId == notebook.Id);
                ...
                var authorizations = _db.NotebookAuthorizedUsers.Where(x => x.NotebookId == notebook.Id && x.UserId == userId);
                _db.NotebookAuthorizedUsers.RemoveRange(authorizations);
                await _db.SaveChangesAsync();
            }
            var notebookToDelete = _db.Notebooks.Where(x => typesDeleted.Contains(x.Id));
            _db.Notebooks.RemoveRange(notebookToDelete);
            await _db.SaveChangesAsync();
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest model)
        {
            ...
            //Example: Only allow users who's email address ends with a '@yourcompanydomain.com'
            ...
            if (result.Succeeded)
            {
                ...
                //Any logic for new users, put here
                await _db.SaveChangesAsync();
